...
/**
 * 아케이드(포인트/전적) 대시보드. 원래 /arcade/profile 페이지였는데,
 * 쿠지허브 마이페이지(/profile)에 섹션으로 통합되었다.
 */
export default function ArcadePointsPanel() {
  ...
  if (!stats) return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '48px 0' }}>
      <ArcadeBox label="PLAYER_CARD" variant="primary" style={{ textAlign: 'center', padding: '32px 48px' }}>
        <p style={{ color: '#fff', fontWeight: 700, fontSize: '1.05rem' }}>
          포인트·전적 데이터를 보려면 아케이드 계정으로 로그인해주세요. (INSERT COIN)
        </p>
      </ArcadeBox>
    </div>
  )
